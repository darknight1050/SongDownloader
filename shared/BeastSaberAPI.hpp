...
namespace BeastSaber::API {
    ...
    SONGDOWNLOADER_EXPORT std::optional<BeastSaber::Page> CuratorRecommended(int pageIndex = 0, int amount = 20);
    ...
    /// <param name="amount">Amount of songs to request.</param>
    /// <returns>Returns an std::optional that can either contain the results or std::nullopt if no results were found or an error encountered</returns>
    SONGDOWNLOADER_EXPORT std::optional<BeastSaber::Page> Bookmarked(std::string username, int pageIndex = 0, int amount = 20);
    ...
    SONGDOWNLOADER_EXPORT void CuratorRecommendedAsync(std::function<void(std::optional<BeastSaber::Page>)> finished, int pageIndex = 0, int amount = 20);
    ...
    /// <param name="pageIndex">Index of the page you want to request. Default: 0</param>
    /// <param name="amount">Amount of songs to request</param>
    SONGDOWNLOADER_EXPORT void BookmarkedAsync(std::string username, std::function<void(std::optional<BeastSaber::Page>)> finished, int pageIndex = 0, int amount = 20);
